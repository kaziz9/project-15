...
interface EditLinkModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdateLink: (id: string, link: Omit<Link, 'id' | 'createdAt'>) => void;
  ...
  const fetchPreview = async () => {
    ...
    try {
      ...
      if (domain.includes('youtube.com') || domain.includes('youtu.be')) {
        ...
        if (videoId) {
          setTitle(`${language === 'ar' ? 'فيديو يوتيوب محدث' : 'Updated YouTube Video'} - ${videoId}`);
          setDescription(language === 'ar' ? 'فيديو محدث من يوتيوب - يمكنك تعديل العنوان والوصف' : 'Updated YouTube video - you can edit the title and description');
          setImage(`https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`);
        }
      } else {
        // For other websites, use a generic preview
        setTitle(`${language === 'ar' ? 'مقال محدث من' : 'Updated article from'} ${domain}`);
      ...
      setTitle(language === 'ar' ? 'رابط محدث' : 'Updated Link');
      ...
      <div className={`w-full max-w-2xl rounded-xl shadow-2xl max-h-[90vh] overflow-y-auto ${
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            {imageUploadMethod === 'upload' && (
              <div>
                {image && imageUploadMethod === 'upload' && image.startsWith('data:') ? (
